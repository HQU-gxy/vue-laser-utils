...
/* {
    deviceKeys: {
    nwkKey: "e11e195840ec0369b9c0902b6d817cc8",
    devEUI: "6c60d6c41c5c144c"
  }
} */
export class ChirpStackApi {
  ...
  constructor(api_info?: ChirpStackApiConfig) {
    const default_api_info = {
      ...
      token:
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhcGlfa2V5X2lkIjoiODliNGVhM2YtNGNlYS00NDcyLTllMjItYmVlNDY4MWUzOThmIiwiYXVkIjoiYXMiLCJpc3MiOiJhcyIsIm5iZiI6MTYyMzczNDQwNCwic3ViIjoiYXBpX2tleSJ9.J6JZYcMYYtPXSFjGs2RsVW6k7-r7bU8OHHdKMH1UESM",
      applicationID: "2",
    }
    this.config = api_info
      ? { ...default_api_info, ...api_info }
      : default_api_info
  ...
  public async postDevice(device: LoraWanDevice): Promise<AxiosResponse> {
    // omit appKey
    const postContent = { device: (({ appKey, ...obj }) => obj)(device) }
    return new Promise((resolve, reject) => {
      this.axios
        .post("/devices", postContent, {
          params: { applicationID: device.applicationID },
        })
  ...
  public async postDeviceKey(device: LoraWanDevice): Promise<AxiosResponse> {
    ...
    return new Promise((resolve, reject) => {
      this.axios
        .post(`/devices/${device.devEUI}/keys`, postContent, {
          params: { applicationID: device.applicationID },
        })
        .then((response) => resolve(response))
        .catch((err) => reject(err))
